...
class MemoryController {
  ...
  /**
   * Regenerate file entities (POST /v1/memory/regenerate-file)
   */
  async regenerateFile(req, res) {
    try {
      const regenerateFile = this.locator.get('RegenerateFile');
      const { file_path, project_root, group_id } = req.body;
      ...
      logger.error({ err: error }, 'RegenerateFile error');
      res.status(400).json({
        success: false,
        error: error.message
      });
