  ...
  try {
    ...
    switch (name) {
      ...
      case 'recall': {
        ...
        const recallResults = await recallEntities.execute({
          query: args.query,
          group_id: args.group_id,
          type: args.type,
        ...
        // Check for cross-group results and add notice
        const crossGroupResults = recallResults.filter(r => r.cross_group_match);
        if (crossGroupResults.length > 0) {
          const groups = [...new Set(crossGroupResults.map(r => r.group_id))];
          result = {
            notice: `⚠️ Found ${crossGroupResults.length} result(s) in different group(s): ${groups.join(', ')}. ` +
              `You requested '${args.group_id}' but matches were found in other projects. Use the correct group_id for better results.`,
    ...
    // Handle wrapped results (e.g., recall with cross-group notice)
